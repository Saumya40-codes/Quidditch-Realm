...
const Team = () => {
  ...
  const getTeam = async () => {
    try {
      ...
      setTeam(res.data);
        ...
        console.log(name);
        const res = await Axios.get(`https://quidditch-realm.vercel.app/teams/name/${name}`)
  ...
  const handleTeamDel = async (req, res) => {
    try {
      const res = Axios.delete(`https://quidditch-realm.vercel.app/teams/del/team/${team._id}`).then((res) => {
